...
ROI = {
    ...
    "close_black": (250, 170, 140, 40),
    "parking_wall": (250, 100, 140, 40),
    "last_wall": (280, 50, 80, 20),
    "last_wall_2": (260, 70, 80, 20),
...
class VisionPipeline:
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, List[Dict]]:
        ...
        for c, area, cbcx, cbcy in self.find_all_above(
            final_park_black, MIN_AREA["parkWall"]
        ):
            results["black"].append(
                {
                    "contour": c + [0, G_Y0],
        ...
        if results["red"] and results["magenta"]:
            ...
            for rd in results["red"]:
                rcx, rcy = rd["centroid"]
                too_close = any(
                    abs(rcx - md["centroid"][0]) < 10
                    and abs(rcy - md["centroid"][1]) < 10
