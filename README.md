...
#### Open Challenge
Iteration 1- LiDAR-based: In our first Open Challenge iteration, we used our 2 TFMini+ LiDAR sensors at the side of the robot to detect the distance from the inner field perimeter. These sensors detected the distance to the nearest object that is in front of them, which in our case were the walls (inner and outer) of the game field. When either of the sensors detected a large jump in the distance to the nearest wall, they recognised that a turn (clockwise or anticlockwise) was required. The servo then turned, until the bot completed a 90 degree turn. After this, the robot continues forward again, until that same sensor detects a large jump in distance. There, the bot turns again. After the robot completes 12 turns (3 rounds), it identifies that it has finished the course. We also used encoder data to track how much distance the robot has travelled so that it can finish in the same zone where it started. We also used multiprocessing in the Pi so that we can run sensor reading, steering correction, and LiDAR processing simultaneously.

Iteration 2 - Vision-based: In our second iteration, we had the Open Challenge program based entirely off the vision input from the Hikvision webcam. We created regions of interest (ROIs), which are digital bounds for where the camera can detect objects, using OpenCV. Within each ROI, we had a specific set of colours that could be detected - for example, in the 'block' region the robot would only take into account a green or red flag. At the start, the robot would just move straight forward until it detected one of the lines. Depending on the colour of the first line detected, the robot would register the direction it is going in (clockwise/anticlockwise), and keep moving forward until the wall in front of it enters the 'near wall' ROI. As soon as it detects that, it makes the turn in the direction based on the line detected earlier. This sequence continues for the whole challenge, and the robot stops once it has completed 12 turns and the 'far wall' ROI has been triggered.

// ADD SCREENSHOT OF OPEN CHALLENGE ROIs HERE
